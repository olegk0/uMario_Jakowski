...
void BulletBill::Draw(CIMG* iIMG) {
    if (minionState != -2) {
        iIMG->Draw((int)fXPos + (int)CCore::getMap()->getXPos(), (int)fYPos + 2, moveDirection);
    } else {
        iIMG->DrawVert((int)fXPos + (int)CCore::getMap()->getXPos(), (int)fYPos);
    }

    if (minionState == 0) CCore::getMap()->getBlock(145)->getSprite()->getTexture()->Draw((int)(currentJumpSpeed * 32 + CCore::getMap()->getXPos()), (int)(CCFG::GAME_HEIGHT - 16 - currentFallingSpeed * 32) + 32);
}
