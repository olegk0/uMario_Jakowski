...
void PauseMenu::Draw() {
    ...
    BlitSquareNorm(&rPause, 4, 4, 4, 235);
    ...
    BlitSquareNorm(&rPause, 155, 155, 155, 255);
    rPause.x -= 1;
    rPause.y -= 1;
    rPause.h += 2;
    rPause.w += 2;
    ...
    for (unsigned int i = 0; i < lMO.size(); i++) {
        if (i == activeMenuOption) {
            CCFG::getText()->DrawCenterX(lMO[i]->getText(), lMO[i]->getYPos(), 16, 255, 255, 255);
        } else {
            CCFG::getText()->DrawCenterX(lMO[i]->getText(), lMO[i]->getYPos(), 16, 90, 90, 90);
        }
    }
