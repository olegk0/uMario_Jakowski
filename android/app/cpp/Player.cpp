...
Player::Player(float fXPos, float fYPos) {
    this->fXPos = fXPos;
    this->fYPos = fYPos;
    this->iNumOfLives = 3;
    ...
    tempI.push_back(0);
    sMario.push_back(new Sprite(tempS, tempI, true));
    tempS.clear();
...
void Player::playerPhysics() {
    if (!CCore::getMap()->getUnderWater()) {
        if (jumpState == 1) {
            ...
            if (!CCFG::keyJump && currentJumpDistance > 64 && !springJump) {
                jumpDistance = 16;
...
void Player::Draw() {
    if (!inLevelDownAnimation || CCore::getMap()->getInEvent()) {
        sMario[getMarioSpriteID()]->getTexture()->Draw((int)fXPos, (int)fYPos + (CCore::getMap()->getInEvent() ? 0 : 2), !moveDirection);
    } else {
        if (inLevelDownAnimationFrameID % 15 < (inLevelDownAnimationFrameID > 120 ? 7 : inLevelDownAnimationFrameID > 90 ? 9
    ...
    if (starEffect && !inLevelAnimation && CCFG::getMM()->getViewID() == CCFG::getMM()->eGame) {
        ...
        if (unKillAbleTimeFrameID < 90) {
            if (unKillAbleFrameID < 5) {
                return powerLVL < 1 ? iSpriteID + 44 : powerLVL == 2 ? iSpriteID
                                                                     : iSpriteID + 11;
            } else if (unKillAbleFrameID < 10) {
                return powerLVL < 1 ? iSpriteID + 55 : powerLVL == 2 ? iSpriteID + 11
                                                                     : iSpriteID + 22;
            } else {
                unKillAbleFrameID = 0;
            }
...
int Player::getHitBoxY() {
    return powerLVL == 0 ? iSmallY : bSquat ? 44
                                            : iBigY;
}
