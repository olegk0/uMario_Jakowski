...
void MainMenu::Draw() {
    CCFG::getSMBLOGO()->Draw(80, 48);
    Menu::Draw();
    CCFG::getText()->Draw("WWW.LUKASZJAKOWSKI.PL", 4, CCFG::GAME_HEIGHT - 4 - 8, 8, 0, 0, 0);
    CCFG::getText()->Draw("WWW.LUKASZJAKOWSKI.PL", 5, CCFG::GAME_HEIGHT - 5 - 8, 8, 255, 255, 255);

    if (selectWorld) {
        // SDL_SetRenderDrawBlendMode(SDL_BLENDMODE_BLEND);
        // SDL_SetRenderDrawColor(4, 4, 4, 235);
        // SDL_RenderFillRect(&rSelectWorld);
        BlitSquareNorm(&rSelectWorld, 4, 4, 4, 235);
        // SDL_SetRenderDrawColor(255, 255, 255, 255);
        rSelectWorld.x += 1;
        rSelectWorld.y += 1;
        rSelectWorld.h -= 2;
        rSelectWorld.w -= 2;
        // SDL_RenderDrawRect(&rSelectWorld);
        BlitSquareNorm(&rSelectWorld, 155, 155, 155, 255);
        rSelectWorld.x -= 1;
        rSelectWorld.y -= 1;
        rSelectWorld.h += 2;
        rSelectWorld.w += 2;

        CCFG::getText()->Draw("SELECT WORLD", rSelectWorld.x + rSelectWorld.w / 2 - CCFG::getText()->getTextWidth("SELECT WORLD") / 2, rSelectWorld.y + 16, 16, 255, 255, 255);

        for (int i = 0, extraX = 0; i < 8; i++) {
            if (i == activeWorldID) {
                CCFG::getText()->Draw(std::to_string(i + 1) + "-" + std::to_string(activeSecondWorldID + 1), rSelectWorld.x + 16 * (i + 1) + 16 * i + extraX, rSelectWorld.y + 16 + 24, 16, 255, 255, 255);

                extraX = 32;

                /*for(int j = 0; j < 4; j++) {
                        if(j == activeSecondWorldID) {
                                CCFG::getText()->Draw( std::to_string(j + 1), rSelectWorld.x + 16*(i + 1) + 16*i, rSelectWorld.y + 40 + 24*j, 16, 255, 255, 255);
                        } else {
                                CCFG::getText()->Draw( std::to_string(j + 1), rSelectWorld.x + 16*(i + 1) + 16*i, rSelectWorld.y + 40 + 24*j, 16, 90, 90, 90);
                        }
                }*/
            } else {
                CCFG::getText()->Draw(std::to_string(i + 1), rSelectWorld.x + 16 * (i + 1) + 16 * i + extraX, rSelectWorld.y + 16 + 24, 16, 90, 90, 90);
            }
        }
