...
void Platform::Draw() {
    if (iType == 6 || iType == 7) {  // -- iXStart = YPos 2
        for (int i = 1; i < (iXStart - iYEnd) / 16 + 2; i++) {
            CCore::getMap()->getBlock(CCore::getMap()->getCurrentLevelID() == 22 ? 165 : 135)->getSprite()->getTexture()->Draw((int)(iXEnd + (int)CCore::getMap()->getXPos() + (iSize * 8) / 2 + 16 - (iSize / 2 % 2 == 0 ? 8 : 0)), (int)(iXStart - 16 * i));
        }
    }

    for (int i = 0; i < iSize; i++) {
        CCore::getMap()->getBlock(iType != 4 ? 74 : 126)->Draw((int)fXPos + i * 16 + (int)CCore::getMap()->getXPos(), (int)fYPos);
    }
}
