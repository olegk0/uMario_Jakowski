...
void LevelEditor::Draw() {
    ...
    if (CCore::mouseRightPressed) {
        ...
        // SDL_SetRenderDrawColor(242, 242, 242, 78);
        // SDL_RenderFillRect(&rDrag);
        BlitSquareNorm(&rDrag, 242, 242, 242, 78);
        // SDL_SetRenderDrawColor(255, 255, 255, 235);
        // SDL_RenderDrawRect(&rDrag);
        BlitSquareNorm(&rDrag, 255, 255, 255, 235);
    } else {
        CCore::getMap()->getBlock(currentBlockID)->getSprite()->getTexture()->Draw(rBlock.x, rBlock.y);
        // SDL_RenderDrawRect(&rBlock);
        BlitSquareNorm(&rBlock, 255, 255, 255, 128);
    }
...
void LevelEditor::drawStruct() {
    ...
    for (int i = 0, k = 0; k < (W > 0 ? W : -W); k += 32, i += W > 0 ? 32 : -32) {
        for (int j = 0, m = 0; m < (H > 0 ? H : -H); m += 32, j += H > 0 ? 32 : -32) {
            CCore::getMap()->getBlock(currentBlockID)->getSprite()->getTexture()->Draw(rBlock.x + i, rBlock.y + j);
        }
    }
}