...
void Text::Draw(std::string sText, int X, int Y, int fontSize) {
    ...
    for (unsigned int i = 0; i < sText.size(); i++) {
        ...
        rRect.w = fontSize;
        rRect.h = fontSize;
        FONT->DrawCrop(rCrop, rRect);
        extraLeft += nextExtraLeft;
        nextExtraLeft = 0;
    }
}

void Text::Draw(std::string sText, int X, int Y, int fontSize, int iR, int iG, int iB) {
    this->fontSize = fontSize;
    this->extraLeft = 0;
    this->nextExtraLeft = 0;

    for (unsigned int i = 0; i < sText.size(); i++) {
        // glClearColor((float)iR / 255.0, (float)iG / 255.0, (float)iB / 255.0, 0.0);
...
void Text::DrawCenterX(std::string sText, int Y, int fontSize, int iR, int iG, int iB) {
    int X = CCFG::GAME_WIDTH / 2 - getTextWidth(sText, fontSize) / 2;

    Draw(sText, X, Y, fontSize, iR, iG, iB);
}

void Text::Draw(std::string sText, int X, int Y, int iWidth, int iHeight) {
    for (unsigned int i = 0; i < sText.size(); i++) {
        rCrop.x = getPos(sText.at(i));
...
void Text::DrawWS(std::string sText, int X, int Y, int iR, int iG, int iB, int fontSize) {
    this->fontSize = fontSize;
    this->extraLeft = 0;
    this->nextExtraLeft = 0;
