...
void AboutMenu::Draw() {
    CCFG::getText()->DrawWS("MARIO V 1.03 - C++ AND SDL2", 150, 128, 0, 0, 0);
    CCFG::getText()->DrawWS("AUTOR: LUKASZ JAKOWSKI", 150, 146, 0, 0, 0);

    CCFG::getText()->DrawWS("INFORMATYKA INZ 2012-2016", 150, 188, 0, 0, 0);
    CCFG::getText()->DrawWS("UNIWERSYTET SLASKI W KATOWICACH", 150, 206, 0, 0, 0);
    CCFG::getText()->DrawWS("MAJ 2014", 150, 224, 0, 0, 0);

    CCFG::getText()->DrawWS("WWW.LUKASZJAKOWSKI.PL", 150, 264, 0, 0, 0);

    // CCFG::getText()->DrawWS( std::to_string(iNumOfUnits), 5, CCFG::GAME_HEIGHT - 21, 0, 0, 0);

    for (unsigned int i = 0; i < lMO.size(); i++) {
        CCFG::getText()->DrawWS(lMO[i]->getText(), lMO[i]->getXPos(), lMO[i]->getYPos(), 0, 0, 0);
    }

    CCFG::getMM()->getActiveOption()->Draw(lMO[activeMenuOption]->getXPos() - 32, lMO[activeMenuOption]->getYPos());
}
...
void AboutMenu::setBackgroundColor() {
    // SDL_SetRenderDrawColor(cR, cG, cB, 255);
    glClearColor((float)cR / 255, (float)cG / 255, (float)cB / 255, 1.0);
}
