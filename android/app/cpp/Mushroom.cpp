...
Mushroom::Mushroom(int iXPos, int fYPos, bool powerUP, int iX, int iY) {
    ...
    this->iBlockID = powerUP ? 2 : CCore::getMap()->getLevelType() == 1 ? 25
                                                                        : 3;
    this->moveSpeed = 2;
...
void Mushroom::Draw(CIMG* iIMG) {
    if (minionState >= 0) {
        iIMG->Draw((int)fXPos + (int)CCore::getMap()->getXPos(), (int)fYPos + 2, false);
        if (inSpawnState) {
            CCore::getMap()->getBlock(CCore::getMap()->getLevelType() == 0 || CCore::getMap()->getLevelType() == 4 ? 9 : 56)->getSprite()->getTexture()->Draw((int)fXPos + (int)CCore::getMap()->getXPos(), (int)fYPos + (32 - inSpawnY) - CCore::getMap()->getMapBlock(iX, iY)->getYPos() + 2, false);
        }
    }
}
