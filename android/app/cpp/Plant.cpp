...
void Plant::Draw(CIMG* iIMG) {
    iIMG->Draw((int)(fXPos + CCore::getMap()->getXPos()) - 2, (int)fYPos - 6, false);

    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 2; j++) {
            CCore::getMap()->getBlock(CCore::getMap()->getMapBlock(X + j, Y - i)->getBlockID())->Draw((int)((X + j) * 32 + CCore::getMap()->getXPos()), CCFG::GAME_HEIGHT - 32 * (Y - i) - 16);
        }
    }
}
