...
void Flag::Draw(CIMG* iIMG) {
    iIMG->Draw((int)(iXPos + CCore::getMap()->getXPos()), iYPos);

    if (iPoints > 0) {
        CCFG::getText()->Draw(std::to_string(iPoints), (int)(iXPos + CCore::getMap()->getXPos() + 42), iYTextPos - 22, 8, 16);
    }
}

void Flag::DrawCastleFlag(CIMG* iIMG) {
    iIMG->Draw((int)(iXPos + CCore::getMap()->getXPos() + castleFlagExtraXPos + 7 * 32 - 14), CCFG::GAME_HEIGHT + 14 - 6 * 32 - castleFlagY);
}
