...
void Goombas::Draw(CIMG* iIMG) {
    if (minionState != -2) {
        iIMG->Draw((int)fXPos + (int)CCore::getMap()->getXPos(), (int)fYPos + 2, false);
    } else {
        iIMG->DrawVert((int)fXPos + (int)CCore::getMap()->getXPos(), (int)fYPos + 2);
    }
}
...
void Goombas::collisionWithPlayer(bool TOP) {
    if (CCore::getMap()->getPlayer()->getStarEffect()) {
        ...
        if (minionState == 0) {
            minionState = 1;
            iBlockID = CCore::getMap()->getLevelType() == 0 || CCore::getMap()->getLevelType() == 4 ? 1 : CCore::getMap()->getLevelType() == 1 ? 9
                                                                                                                                               : 11;
