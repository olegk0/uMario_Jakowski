...
void Vine::Draw(CIMG* iIMG) {
    if (jumpDistance < 32) {
        CCore::getMap()->getMinionBlock(iBlockID - 1)->Draw((int)(iX * 32 + CCore::getMap()->getXPos()), (int)(CCFG::GAME_HEIGHT - 16 - iY * 32 - jumpDistance));
    } else {
        CCore::getMap()->getMinionBlock(iBlockID - 1)->Draw((int)(iX * 32 + CCore::getMap()->getXPos()), (int)(CCFG::GAME_HEIGHT - 16 - iY * 32 - jumpDistance));
        for (int i = 0; i < jumpDistance / 32 - 1; i++) {
            iIMG->Draw((int)(iX * 32 + CCore::getMap()->getXPos()), (int)(CCFG::GAME_HEIGHT + 16 - iY * 32 + i * 32 - jumpDistance), false);
        }
    }
    CCore::getMap()->getBlock(CCore::getMap()->getMapBlock((int)iX, (int)iY)->getBlockID())->getSprite()->getTexture()->Draw((int)(iX * 32 + CCore::getMap()->getXPos()), (int)(CCFG::GAME_HEIGHT - iY * 32 - 16 - CCore::getMap()->getMapBlock((int)iX, (int)iY)->getYPos()));
}
