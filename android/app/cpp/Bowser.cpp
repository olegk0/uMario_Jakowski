...
void Bowser::Draw(CIMG* iIMG) {
    iIMG->Draw((int)(fXPos + CCore::getMap()->getXPos()), (int)fYPos, !moveDirection);

    if (spawnHammer) {
        if (nextHammerFrameID <= 24 && moveDirection) {
            CCore::getMap()->getMinionBlock(61)->getSprite()->getTexture()->Draw((int)(fXPos + CCore::getMap()->getXPos() + 4), (int)(fYPos - 20), moveDirection);
        }
    }
}
